...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.0-dev/Envelope_3/include/CGAL/Env_sphere_traits_3.h $
// $Id: include/CGAL/Env_sphere_traits_3.h a484bfa $
...
public:
  ...
  public:
    ...
    operator()(const Xy_monotone_surface_3& s1, const Xy_monotone_surface_3& s2,
      ...
      // Sign do_inter =
      // CGAL_NTS sign(sqr_d*sqr_d + (1-2*sqr_d)*(sqr_r1+sqr_r2)-2*sqr_r1*sqr_r2);
      ...
      if (CGAL_NTS compare(c1, c2) == EQUAL) {
        if (CGAL_NTS compare(b1, b2) == EQUAL) {
          ...
          Algebraic* xs_end = nt_traits->solve_quadratic_equation(D, E, F, xs);
          std::ptrdiff_t n_xs = xs_end - xs;
      ...
      // and the projection is (a part of) an ellipse
        ...
        // compare(CGAL::Env_sphere_traits_3<Conic_traits_2>::Rational&, int)
        Rational zero(0);
        ...
        if (CGAL_NTS compare(lb, zero) != EQUAL) {
          ...
          Rational B = 2*la*lc*S - lb*(lc*T + la*V - lb*U);
          Rational C = S*lc*lc + lb*(lb*W - lc*V);
          ...
          if (n_inter_points > 0)
            source =
              Alg_point_2(inter_xs[0], -(la*inter_xs[0] + lc) / Algebraic(lb));
          if (n_inter_points == 2) {
            target =
              Alg_point_2(inter_xs[1], -(la*inter_xs[1] + lc) / Algebraic(lb));
            ...
            if (CGAL_NTS compare(Algebraic(la)*x_mid + Algebraic(lb)*y1 +
                                 Algebraic(lc),
                                 Algebraic(0)) == LARGER)
            {
              ...
              CGAL_assertion(CGAL_NTS compare(Algebraic(la)*x_mid +
                                              Algebraic(lb)*y2 + Algebraic(lc),
                                              Algebraic(0)) == LARGER);
          ...
          Rational A = S;
          Rational B = T*inter_x + V;
          Rational C = R*inter_x*inter_x + U*inter_x + W;
          ...
          if (n_inter_points == 2) {
            ...
            if (CGAL_NTS compare(Algebraic(la)*x1 + Algebraic(lb)*y_mid +
                                 Algebraic(lc),
                                 Algebraic(0)) == LARGER)
            {
              ...
              CGAL_assertion(CGAL_NTS compare(Algebraic(la)*x2 +
                                              Algebraic(lb)*y_mid +
                                              Algebraic(lc),
                                              Algebraic(0)) == LARGER);
        ...
        if (n_inter_points < 2) {
          ...
          lval = Algebraic(la)*vtan_ps[1].x() + Algebraic(lb)*vtan_ps[1].y() +
            Algebraic(lc);
  ...
  protected:
                                   ...
                                   const Xy_monotone_surface_3& s2) const
    {
      ...
      Sign res =
        CGAL::sign_a_plus_b_x_sqrt_e_plus_c_x_sqrt_f(Algebraic(c_diff),
                                                     Algebraic(-1),
                                                     Algebraic(1),
                                                     A1,
                                                     A2);
                                    ...
                                    bool compare_on_right) const
  {
    ...
    if (C1 != 0 && C2 != 0) {
      ...
      Sign sign3 = (compare_on_right ? (CGAL_NTS sign(1)) : (CGAL_NTS sign(-1)));

      return sign1 * sign2 * sign3;

    }
    // sphere 2 is on the envelope (both lower & upper)
    else if (C1 != 0 && C2 == 0) return LARGER;
    // sphere 1 is on the envelope (both lower & upper)
    else if (C1 == 0 && C2 != 0) return SMALLER;
    else CGAL_error();
    return EQUAL;
  }
  ...
  Point_2 construct_middle_point(const X_monotone_curve_2& cv) const {
    ...
    // TODO_NEW_DESIGN - this is not implemented in X_monotone_curve_2, but
  ...
  bool is_valid_conic_equation(const NT& r, const NT& s, const NT& t,
    ...
    if (sign_A == POSITIVE) return true;
